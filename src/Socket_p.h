...
namespace Arcus
{
    ...
    public:
        Private()
            : state(SocketState::Initial), next_state(SocketState::Initial), received_close(false), port(0), thread(nullptr)
        {
        }
    ...
    void Socket::Private::receiveNextMessage()
    {
        ...
        if (current_message->state == WireMessage::MessageState::Header)
        {
            ...
            if (signature != ARCUS_SIGNATURE)
            {
                ...
                std::cout << "current message:" << std::hex << current_message << std::endl;
